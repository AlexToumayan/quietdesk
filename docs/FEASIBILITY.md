...
Date: 2026-09-17. Test machine: macOS 26.6.2 (Tahoe), Finder 26.4, Apple Silicon, Command Line
...
("Show Items > On Desktop"), and draw the icons ourselves in a transparent window just above the
...
the native desktop on disable, quit and after a crash, and idles at about a thousandth of one core (0.0 % CPU in every sample, E15). The price is that, while
...
decision this milestone was meant to inform. (The owner chose to build it; see section 7.)
...
| Custom layer just above `kCGDesktopIconWindowLevel` (shield at + 1, icon window at + 2) drawing icons and labels, Finder items hidden by the setting above | Surface (documented window APIs) | Full control of labels; icons where Finder would put them. | **Prototype built and measured.** | Section 3–5, experiments E4/E6–E9. |
...
- **E7 — layout reproduction.** The prototype's computed order and cell positions matched a screenshot of the test desktop column for column: a volume, 8 date stacks, then 80 folders in Finder's order, 13 rows, 7 columns, last column ending at row 11. Cell pitch 84x82 pt, first icon centre at (1677, 60).
...
- **E9 — resource use.** Release build, real mode (native items hidden, overlay shown), no interaction: CPU time constant at 0.13 s across a 40 s run (0.0 % in every `ps` sample), resident size 83 MB stable (65–77 MB in shorter runs). During runs where the overlay was interacted with, CPU rose by ~0.3 s and resident size by up to 28 MB; the icon cache now pre-renders one small bitmap per icon to bound that. At the time of this measurement the overlay had no timers and no polling, and it read the disk after the first scan only on a Desktop folder change, a volume mount or unmount, or a display change. A once-a-second window-list check was added later (E14, E15).

- **E10 — NSPanel level trap.** Switching the overlay to a non-activating `NSPanel` (so keyboard focus works without activating the app) silently reset its level to normal because `isFloatingPanel` rewrites `level`; a 12 s run put the overlay above application windows. Fixed by setting `isFloatingPanel` first and guarding the `level` setter; hit tests at the time reported level -2147483602. The icon panel has since moved one level up, to -2147483601.
...
- **E13 — grid calibration, second and third points.** With Finder's grid-spacing slider at its minimum, Finder stores `gridSpacing = 1` and draws 50 x 66 pt cells for icon 36 / text 12 (measured from a screenshot); at 26 it draws 84 x 82. A third point, icon 32 at spacing 1, measured 48 x 60 pt. The two-point formula was 2 pt too narrow and 2 pt too tall there, so the formula was refitted to all three points (`GridMetrics.from` in `Layout.swift`; `--self-test` checks all three). Also found: reading another app's preferences returns a per-process cached copy unless `CFPreferencesAppSynchronize` is called first, which is why a freshly changed Finder setting was missed once.
...
- **E14 — revealing the desktop (Show Desktop).** Four findings, all with the hide key set. (1) However a reveal is started (wallpaper click, or the Dock asked directly), macOS re-shows Finder's hidden desktop items for as long as the desktop is revealed; a screenshot taken during a programmatic reveal shows every native icon with its name. So an overlay left on screen during a reveal produces doubled icons, which also happened with F11 and the spread gesture before this was handled. (2) Nothing announces a reveal: a probe listening to every distributed and workspace notification, seven candidate Darwin notifications, and the occlusion, expose, screen and move notifications of a desktop-level panel recorded nothing at the start or the end. The only observable is in the window list: while revealed the Dock owns one screen-sized window per display at layer 18, and WindowManager's click-catchers are gone; one check costs about 1 ms. (3) The old `Mission Control 1` command line no longer does anything on macOS 26; `CoreDockSendNotification("com.apple.showdesktop.awake")`, a private symbol found with `dlsym`, toggles Show Desktop without any permission. (4) QuietDesk's desktop-level panels stay on screen during a reveal unless ordered out. Scripts: `experiments/reveal-trigger.swift`, `reveal-probe.swift`, `reveal-events-probe.swift`. Consequence: QuietDesk checks the window list once a second (four times a second during a reveal), orders its windows out while the desktop is revealed, and a plain wallpaper click asks the Dock to reveal the desktop when System Settings has that gesture on.

- **E15 — resource use with the reveal check.** Release build, real mode, private settings suite, no interaction, 60 s: 0.0 % CPU in every `ps` sample after launch, cumulative CPU time 0.25 s at 5 s and 0.30 s at 55 s (the once-a-second window-list check, about 1 ms each), resident 84 MB and flat. Building the View Options panel on first use instead of at launch took 7 MB off (91 to 84 MB). Earlier figures (E9, E12) were taken before the compact grid, the panel and the reveal check existed and are kept as history.

Not measured: GPU work (no tools without Xcode), behaviour during Mission Control, Stage Manager, full-screen apps, sleep/wake and display reconfiguration (see the checklist), and the .app bundle's own permission prompts (the experiments ran the bare executable, whose prompts are attributed to the parent process).
...
3. Order entries the way Finder fills a sorted desktop (volumes, then Stacks newest bucket first, then folders/files by the sort key, ties by name) and lay them out from the top-right of the main display downward, then leftward, then on the next display. Cell size follows a formula fitted to three cells measured from Finder at text size 12: 84 x 82 pt at icon 36 and spacing 26, 50 x 66 pt at icon 36 and spacing 1, and 48 x 60 pt at icon 32 and spacing 1 (E13). Other settings are estimated from those three. The exact formula is `GridMetrics.from` in `Layout.swift`, and `--self-test` checks the three measured cells.
4. Per display, show a bitmap-free full-screen shield window (a single colour layer at alpha 1/255) and, above it, a window sized to the icon grid that draws icons and labels. Both are borderless, non-activating `NSPanel`s just above Finder's icon level, with `canJoinAllSpaces`, `stationary`, `ignoresCycle`. The shield sits at `kCGDesktopIconWindowLevel + 1` (-2147483602) and the icon window at `+ 2` (-2147483601). Two separate levels mean AppKit can never put the shield above the icons. Non-activating means the icon panel can take keyboard focus while Finder stays the active application, so a desktop click brings Finder's menu bar forward as it does natively (switchable in the menu).
...
8. Once a second, read the window list to see whether the desktop has been revealed (E14). macOS sends no event for a reveal, so this is the only way to notice one. The timer has half a second of tolerance, runs four times a second while the desktop is revealed, and pauses while the displays sleep.
...
Revealing the desktop (E14): a plain click on the wallpaper asks the Dock to Show Desktop when System Settings › Desktop & Dock › "Click wallpaper to reveal desktop" allows it, through a private Dock entry point that is looked up at run time and fails safe (without it the click only deselects). macOS re-shows Finder's own items during any reveal, so QuietDesk orders its windows out while the desktop is revealed and returns when the reveal ends. A reveal that QuietDesk started itself (a wallpaper click) is noticed in well under a second. One started by F11, the spread gesture or a hot corner is noticed at the next once-a-second check, so doubled icons can show for up to about a second and a half. What you see while revealed is Finder's desktop, names included, exactly as macOS shows it to anyone who hides desktop items. Keeping QuietDesk's quiet icons during a reveal would need Finder's desktop switched off altogether (`CreateDesktop`), which takes a Finder restart on every enable and disable and is ruled out by the brief.

Deliberately not offered: Finder's own View Options panel for the desktop (QuietDesk has its own panel, ⌘J, with the same controls and a few of its own; its changes apply to QuietDesk's desktop only and never touch Finder's settings); Finder's menu-bar commands acting on QuietDesk's selection (they act on Finder's hidden desktop, which has no selection).
