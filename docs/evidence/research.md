...
| FinderSync.framework still ships on 26.6.2 and two third-party Finder Sync extensions are registered (one enabled, one not) | none (local) | medium | supported (re-run read-only) | local `pluginkit -m -v -p com.apple.FinderSync` |
