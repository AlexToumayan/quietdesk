...
- **A relayout-under-the-pointer bug**, reported by the owner as "nothing opens any more" after using View Options: with a Stack open, the first click of a double-click collapsed it, the grid shifted, and the second click landed on whatever had moved there. Reading the code did not find it; a scenario test that sends synthesized clicks through the real overlay windows (346 checks across 19 View Options and activation states) reproduced it in every state, the fix remembers what the first click hit, and CI now runs that test on a fixture desktop so it cannot come back.
